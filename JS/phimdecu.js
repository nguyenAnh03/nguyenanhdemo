...
//     { ten: 'Thưa mẹ con đi', nam: 2019, theloai: ['Tâm lí - Tình cảm'], id: 'thuamecondi' },
// ];
import {listMovieOffer} from "./phimdecu-info.js";
...
function changepage(numberindex, totalItem) {
    ...
    for (let i = numberindex; i < totalItem; i++) {
        var content = `<section class="item-phimdecu">
                            ...
                            <div class="title-phim watch-movie">${listMovieOffer[i].ten}</div> 
        ...
        document.getElementsByClassName('watch-movie').onclick = function () {
            watchmovie(i);
        }
    }
...
function watchmovie(j) {
    var listItem = document.getElementsByClassName('watch-movie');
